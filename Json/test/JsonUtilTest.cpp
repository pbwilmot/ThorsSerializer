...
TEST(JsonUtil, MergeArrays)
{
    ...
    std::unique_ptr<JsonArray>    array1 = SMART_OWNED_MOVE(scanner.getArray());

    std::stringstream   data2("[1, 2, 3, 4]");
    scanner.parse<yy::ParserShiftReduce>(data2);
    std::unique_ptr<JsonArray>    array2 = SMART_OWNED_MOVE(scanner.getArray());
...
TEST(JsonUtil, MergeMaps)
{
    ...
    scanner.parse<yy::ParserShiftReduce>(data1);
    std::unique_ptr<JsonMap>    map1 = SMART_OWNED_MOVE(scanner.getMap());

    std::stringstream   data2("{ \"item5\": 1, \"item6\": 2, \"item7\": 3, \"item8\": 4}");
    scanner.parse<yy::ParserShiftReduce>(data2);
    std::unique_ptr<JsonMap>    map2 = SMART_OWNED_MOVE(scanner.getMap());
